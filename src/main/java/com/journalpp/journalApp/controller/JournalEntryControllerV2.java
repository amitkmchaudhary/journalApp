...
public class JournalEntryControllerV2 {
    ...
    public ResponseEntity<JournalEntry> getEnrtybyid(@PathVariable ObjectId myId){
        ...
        String userName = authentication.getName();
        User user = userService.FindByUserName(userName);
        List<JournalEntry> collector = user.getJournalEntries().stream().filter(x->x.getId().equals(myId)).toList();
    ...
    public ResponseEntity<?> deleteEnrtybyid(@PathVariable ObjectId myId){
        Authentication authentication = SecurityContextHolder.getContext().getAuthentication();
        String userName = authentication.getName();
